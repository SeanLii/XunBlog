# XunBlog v6 — Canonical Independence & Provenance Audit
...
v6 不再使用“被多个模型使用 → 自动提升为公共 topic”的规则。  
...
v6 对全部 82 个 content pages 同时执行两类审查：

1. canonical independence / provenance；
2. content completeness / formality。
...
- 0 个内部断链；
- 0 个多 H1 页面；
- 0 个 H1 / frontmatter title mismatch；
- 0 个问题型 H2/H3；
- 0 个非法控制字符。

唯一保留的问题型页面标题是 `为什么 ACT 推理时令 z = 0？`，因为页面本身就是 ACT-specific question。

本文件只记录 ownership / provenance 判定。82 页逐页内容处理结果见 `CONTENT_PAGE_AUDIT.md`；v6 不再默认保护 ACT、Transformer 或 π0，三者同样按正式教材、内容完整性和 paper/code grounding 标准重新审查。
...
为避免让 CLS 成为“无父模型的背景页”，ownership refactor 建立完整 `BERT` canonical page。
...
## ACT ownership boundary

ACT 作为模型属于 Robot Learning。其模型特定设计继续保留在 ACT 下，包括：

- Action Chunking（ACT 的 robot-policy formulation）；
- Temporal Ensemble；
- Architecture / Training / Inference / Complete Data Flow；
- CVAE in ACT；
- Vision Pipeline；
...
- ResNet；
- Transformer；
- BERT / CLS-style readout；
- DETR / Object Query lineage；
- CVAE。

## Action Chunking scope

ACT 论文明确说明其设计受到 neuroscience action chunking 概念启发，因此不声称 ACT 发明了所有意义上的 chunking。当前页面范围限定为：

\[
\pi(a_{t:t+k-1}\mid o_t),
\]

即 ACT 中根据当前 observation 一次预测 future action sequence、用于降低 effective horizon 的 robot imitation-learning formulation。

## Paper / implementation boundary

ACT 的 `Paper and Released Implementation` 页面继续显式保留 reconstruction-loss wording、optimizer、released-code behavior 等差异，不为了叙述整齐统一成单一版本。

---

# 7. π0 Audit

## π0 ownership boundary

π0 自己拥有的 model-specific topics 包括：
...
- Training / Inference / Complete Data Flow；
- Paper and Released Implementation。
...
Flow Matching 作为通用生成建模方法独立存在；`Flow Matching in π0` 只解释 π0 的 conditional action path、vector-field target、time conditioning 与 numerical sampling。

## Paper / released implementation boundary

π0 paper 与 official `openpi` 的 flow-time convention 方向相反。v6 继续分别记录两套 convention，并把 prefix KV-cache 等 released behavior 明确标为 implementation detail。

---

# 8. Canonical Pages Established by the Ownership Refactor

为修复旧页面“把来源吞进 background”的问题，前序 ownership refactor 新增或正式建立，并在 v6 中继续保留：
